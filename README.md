<div align="center">

<a href="https://ijk37.github.io/network-systems/03-quiz/"><img src="assets/banner.svg" alt="Network Systems — CompTIA Network+" width="100%"></a>

<p>
  <a href="01-notes/README.md"><img src="https://img.shields.io/badge/Notes-12_modules-B4122E?style=for-the-badge&labelColor=7E0E23" alt="Course Notes"></a>
  <a href="02-exercises/README.md"><img src="https://img.shields.io/badge/Exercises-12_sets-B4122E?style=for-the-badge&labelColor=7E0E23" alt="Exercises"></a>
  <a href="https://ijk37.github.io/network-systems/03-quiz/"><img src="https://img.shields.io/badge/Quiz_Hub-Open-C8901A?style=for-the-badge&labelColor=7E0E23" alt="Quiz Hub"></a>
  <a href="04-projects/README.md"><img src="https://img.shields.io/badge/Projects-13_labs-B4122E?style=for-the-badge&labelColor=7E0E23" alt="Projects"></a>
</p>

<p>
  <img src="https://img.shields.io/badge/Question_Bank-~1,200-C8901A?style=flat-square&labelColor=7E0E23" alt="Question Bank">
  <img src="https://img.shields.io/badge/Modules-12-B4122E?style=flat-square&labelColor=7E0E23" alt="Modules">
  <img src="https://img.shields.io/badge/Projects-13-B4122E?style=flat-square&labelColor=7E0E23" alt="Projects count">
  <img src="https://img.shields.io/badge/Exam-N10--008-C8901A?style=flat-square&labelColor=7E0E23" alt="Exam">
</p>

<p><em>A structured, self-paced learning repository built around <strong>CompTIA Network+ Guide to Networks, 9th Edition</strong> by Jill West.</em></p>

</div>

---

## Course Dashboard

| Section | Purpose | Start Here |
| --- | --- | --- |
| **01 Notes** | Topic-based study notes organized by module | [Browse notes](01-notes/README.md) |
| **02 Exercises** | Practice questions and worked activities | [Open exercises](02-exercises/README.md) |
| **03 Quiz** | Interactive multiple-choice review quizzes | [Launch quiz hub](https://ijk37.github.io/network-systems/03-quiz/) |
| **04 Projects** | Packet Tracer, design, documentation, and analysis labs | [View projects](04-projects/README.md) |
| **05 Resources** | Reserved for supplemental resources | Hidden for now |

## Learning Path

```text
Read notes -> Practice exercises -> Take quiz -> Build project -> Review weak areas
```

1. Start with the module notes in [`01-notes`](01-notes/README.md).
2. Complete the matching exercise in [`02-exercises`](02-exercises/README.md).
3. Test recall in the [Quiz Hub](https://ijk37.github.io/network-systems/03-quiz/).
4. Apply the topic through a lab or design task in [`04-projects`](04-projects/README.md).

## Module Map

| Module | Topic | Primary Reading |
| --- | --- | --- |
| 01 | Introduction to Networking | [What Is a Network?](01-notes/01-01-what-is-network.md) |
| 02 | Infrastructure and Documentation | [Structured Cabling](01-notes/02-01-structured-cabling.md) |
| 03 | Addressing | [IPv4 Addressing](01-notes/03-02-ipv4-addressing.md) |
| 04 | Network Protocols | [TCP/IP Suite](01-notes/04-01-tcp-ip-suite.md) |
| 05 | Cabling | [Copper Cabling](01-notes/05-01-copper-cabling.md) |
| 06 | Wireless Networking | [Wireless Standards](01-notes/06-01-wireless-standards.md) |
| 07 | Cloud Computing and Remote Access | [Cloud Service Models](01-notes/07-01-cloud-service-models.md) |
| 08 | Subnets and VLANs | [Subnetting Basics](01-notes/08-06-subnetting-basics.md) |
| 09 | Network Risk Management | [Security Concepts](01-notes/09-01-security-concepts.md) |
| 10 | Security in Network Design | [Firewalls and ACLs](01-notes/10-01-firewalls-and-acls.md) |
| 11 | Network Performance and Recovery | [Performance Concepts](01-notes/11-01-performance-concepts.md) |
| 12 | Wide Area Networks | [WAN Fundamentals](01-notes/12-01-wan-fundamentals.md) |

## Quiz Hub

The quiz system includes chapter quizzes and cumulative mixed quizzes with instant scoring and end-of-quiz review.

| Feature | Details |
| --- | --- |
| Chapter coverage | 12 module quizzes |
| Question pool | Around 1,200 chapter questions |
| Attempt style | Randomized questions and shuffled options |
| Mixed review | 5 cumulative mixed quizzes |
| Access | [Open the Quiz Hub](https://ijk37.github.io/network-systems/03-quiz/) |

## Projects

The project track turns concepts into implementation and design practice.

| Project | Focus |
| --- | --- |
| [VLSM Network Design](04-projects/01-vlsm-network-design/README.md) | Address planning and subnet allocation |
| [VLANs and Inter-VLAN Routing](04-projects/02-vlans-inter-vlan-routing/README.md) | Segmentation and routing |
| [Static Routing and OSPF](04-projects/03-routing-static-and-ospf/README.md) | Routing design and verification |
| [DHCP, DNS, and Web Services](04-projects/04-dhcp-dns-web-services/README.md) | Core network services |
| [Secure Network Design](04-projects/05-secure-network-design/README.md) | Defense-oriented architecture |
| [Wireshark Traffic Analysis](04-projects/06-wireshark-traffic-analysis/README.md) | Packet inspection and troubleshooting |
| [Network Documentation](04-projects/07-network-documentation/README.md) | Diagrams, inventory, and operational records |
| [Structured Cabling Plan](04-projects/08-structured-cabling-plan/README.md) | Physical layer planning |
| [Wireless Network Design](04-projects/09-wireless-network-design/README.md) | Coverage, channels, and security |
| [Cloud and VPN Connectivity](04-projects/10-cloud-and-vpn-connectivity/README.md) | Remote access and hybrid connectivity |
| [High Availability and Monitoring](04-projects/11-high-availability-and-monitoring/README.md) | Resilience and observability |
| [Network Troubleshooting and Incident Response](04-projects/12-troubleshooting-incident-response/README.md) | Outage triage, evidence collection, and incident reporting |
| [IPv6 Dual-Stack Migration](04-projects/13-ipv6-dual-stack-migration/README.md) | IPv6 addressing, routing, DNS, and security validation |

## Prerequisites

| Tool | Used For |
| --- | --- |
| Cisco Packet Tracer | Network simulation and lab projects |
| Wireshark | Traffic capture and packet analysis |
| Markdown reader or GitHub | Reading notes and exercises |
| Basic computer literacy | Command-line practice, troubleshooting, and documentation |

## Repository Notes

- Numbering follows the 12-module course order.
- Module 08 covers subnets and VLANs, one of the heavier exam areas.
- `05-resources` is intentionally not surfaced here right now. Specific files can be added later when they are ready to share.
- The repository is designed for repeated study: read, practice, test, build, then revisit weak topics.

---

<div align="center">

<strong>Study the notes. Drill the exercises. Prove it in the labs.</strong>

</div>
